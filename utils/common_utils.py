...
def initialize_session(): # 세션 상태 초기화
    default_states = {
        "page": "Home",
        "marked_words": [],
        "records": [],
...
def get_credentials_from_secret_manager(): # 구글 서비스 계정 인증
    secret_name = "google_credentials"  
    secret_data = st.secrets[secret_name]
    credentials_json = secret_data["credentials_json"]  # toml 파일에 저장된 정보
    return Credentials.from_service_account_info(credentials_json)  

def load_google_credentials(): # 구글 서비스 계정 인증 로드
    try:
...
def find_file_in_drive(filename, drive_service): # 구글 드라이브에서 파일 검색
